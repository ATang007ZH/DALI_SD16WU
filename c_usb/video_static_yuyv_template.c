...
#include "../uvc_app/dm1716a_config.h"
...
void video_send(void *addr, uint32_t size)
{
    if (tx_flag) {
        iso_tx_busy = true;
        usbd_video_stream_start_write(0, VIDEO_IN_EP, &packet_buffer[0][0], &packet_buffer[1][0], MAX_PACKETS_IN_ONE_TRANSFER * MAX_PAYLOAD_SIZE, (uint8_t *)addr, size);
#if 0
        while (iso_tx_busy) {
            if (tx_flag == 0) {
                break;

...
Additional information:
  ...
  The data transfer via DMA works only if the buffer that stores the
  data and the internal buffer of the HW layer (the _Dummy variable)
  are located in any other memory region than TCMSRAM.
  The reason for this is that the DMA is not able to access TCMSRAM.
  The HW layer checks for this condition before any data transfer
  and if the condition is not met it transfers the data via CPU instead.
*/
...
*    Checks if the data is located in the tightly-coupled memory.
*
*  Parameters
*    pData      Memory block to be checked.
*
*  Return value
*    !=0      Address is located in TCM.
*    ==0      Address is not located in TCM.
*
*  Additional information
*    TCM is not accessible via DMA.
*/
static int _IsTCM(const void * pData) {
  if (   ((U32)pData >= 0x10000000uL)
      && ((U32)pData <  0x20000000uL)) {
    return 1;
  }
  return 0;
}
